...
class MyCustomFormtxt extends StatefulWidget {
  const MyCustomFormtxt({Key? key}) : super(key: key);

  @override
  MyCustomFormtxtState createState() {
    return MyCustomFormtxtState();
  }
}

class MyCustomFormtxtState extends State<MyCustomFormtxt> {
  final _formKey = GlobalKey<FormState>();
  late TextEditingController controllertxt;
  ...
  void initUser() {
    controllertxt = TextEditingController();
  }
                    ...
                    MaterialButton(
                      ...
                      onPressed: () async{
                        if (_formKey.currentState!.validate()) {
                          ...
                          History newHis = History(id: _hisId, oper: 'Generate', txt: controllertxt.text, date: '${DateTime.now().year}-${DateTime.now().month}-${DateTime.now().day}');
                          _hisId = await _dbHelper.insertHis(newHis);
                          Navigator.push(
                            context,
                            MaterialPageRoute(
                                builder: (context) => Generate(val: controllertxt.text)),
                          ).then(
                            (value) {
                              setState(() {});
                            },
