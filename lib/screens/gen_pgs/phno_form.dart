...
class MyCustomFormphno extends StatefulWidget {
  const MyCustomFormphno({Key? key}) : super(key: key);

  @override
  MyCustomFormphnoState createState() {
    return MyCustomFormphnoState();
  }
}

class MyCustomFormphnoState extends State<MyCustomFormphno> {
                    ...
                    MaterialButton(
                      ...
                      onPressed: () async{
                        if (_formKey.currentState!.validate()) {
                          ...
                          History newHis = History(id: _hisId, oper: 'Generate', txt: 'tel: ${controllerno.text}', date: '${DateTime.now().year}-${DateTime.now().month}-${DateTime.now().day}');
                          _hisId = await _dbHelper.insertHis(newHis);
                          Navigator.push(
                            context,
                            MaterialPageRoute(
                                builder: (context) => Generate(val: 'tel: ${controllerno.text}')),
                          ).then(
                            (value) {
                              setState(() {});
                            },
